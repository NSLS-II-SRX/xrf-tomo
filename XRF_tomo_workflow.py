        ...
        for i in range(num):
            ...
            with h5py.File(fn, "r") as tmp_f:
                if (flag_first):
                    ...
                    f_raw = f.create_dataset("/exchange/raw", data=raw, maxshape=(num, *raw.shape[1:]),
                                             compression="gzip")
                    ...
                    f_x = f.create_dataset("/exchange/x", data=x, maxshape=(num, *x.shape[1:]),
                                           compression="gzip")
                    f_x.resize(num, axis=0)
                    f_y = f.create_dataset("/exchange/y", data=y, maxshape=(num, *y.shape[1:]),
                                           compression="gzip")
                    f_y.resize(num, axis=0)
                    f_i0 = f.create_dataset("/exchange/i0", data=i0, maxshape=(num, *i0.shape[1:]),
                                            compression="gzip")
                    f_i0.resize(num, axis=0)
                    f_fit = f.create_dataset("/reconstruction/fitting/data", data=xrf_fit,
...
def align_proj_com(fn, element='all'):
    ...
    with h5py.File(fn, "r+") as f:
        ...
        for i in range(N_th):
            ...
            if (np.isfinite(tmp_com[0]) == False):
                tmp_com[0] = If.shape[0] / 2
            if (np.isfinite(tmp_com[1]) == False):
...
def moving_translate_alignment():
    ...
    for i in np.arange(45, 132-1):
        ...
        print(shift)
        I = proj[i+1, :, :]
        I = fourier_shift(np.fft.fftn(I), shift)
        I = np.fft.ifftn(I)
        proj[i+1, :, :] = I
...
def normalize_projections(fn, path=None):
    ...
    with h5py.File(path+fn, 'a') as f:
        ...
        try:
            f.create_dataset("reconstruction/recon/proj",
                             data=proj,
                             compression="gzip")
...
def make_volume(fn, path=None, algorithm='gridrec'):
    ...
    with h5py.File(path+fn, 'a') as f:
        ...
        for i in range(N):
            ...
            el_recon = tomopy.recon(el_proj, th, center=rot_center,
